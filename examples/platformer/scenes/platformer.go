...
func (ps *PlatformerScene) init() {
	...
	dresolv.Add(
		space,
		factory.CreateWall(ps.ecs, resolv.NewObject(0, 0, 16, gh)),
		factory.CreateWall(ps.ecs, resolv.NewObject(gw-16, 0, 16, gh)),
		factory.CreateWall(ps.ecs, resolv.NewObject(0, 0, gw, 16)),
		factory.CreateWall(ps.ecs, resolv.NewObject(0, gh-24, gw, 32)),
		factory.CreateWall(ps.ecs, resolv.NewObject(160, gh-56, 160, 32)),
		factory.CreateWall(ps.ecs, resolv.NewObject(320, 64, 32, 160)),
		factory.CreateWall(ps.ecs, resolv.NewObject(64, 128, 16, 160)),
		factory.CreateWall(ps.ecs, resolv.NewObject(gw-128, 64, 128, 16)),
		factory.CreateWall(ps.ecs, resolv.NewObject(gw-128, gh-88, 128, 16)),
	)

	// Create the Player. NewPlayer adds it to the world's Space.
	dresolv.Add(space, factory.CreatePlayer(ps.ecs))

	// Create the floating platform.
	dresolv.Add(factory.CreateFloatingPlatform(ps.ecs, resolv.NewObject(128, gh-32, 128, 8)))

	// Non-moving floating Platforms.
	dresolv.Add(space,
		factory.CreatePlatform(ps.ecs, resolv.NewObject(352, 64, 48, 8)),
		factory.CreatePlatform(ps.ecs, resolv.NewObject(352, 64+64, 48, 8)),
		factory.CreatePlatform(ps.ecs, resolv.NewObject(352, 64+128, 48, 8)),
		factory.CreatePlatform(ps.ecs, resolv.NewObject(352, 64+192, 48, 8)),
	)
